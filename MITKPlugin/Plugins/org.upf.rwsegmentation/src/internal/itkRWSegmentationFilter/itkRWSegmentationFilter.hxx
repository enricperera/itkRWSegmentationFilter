...
namespace itk
{
  ...
  void
  RWSegmentationFilter< TInputImage, TOutputImage >
  ::GenerateData()
  {
    ...
    */
    ...
    std::vector<float> nodes(totalNodes); /* Pixel intensity for all nodes/pixels */
    std::vector<float> labels(totalNodes); /* Label of each node/pixel */
    ...
    */
    std::vector<int> previousFound(totalNodes); 
    std::vector<int> unmarked(unmarkedLength); /* Indices of unmarked nodes ordered */
    std::vector<int> marked(markedLength); /* Store labels of marked nodes */
    std::vector<int> markedIdx(markedLength); /* Indices of marked nodes ordered */
    std::vector<int> nameLabels; /* Values of the different labels of the prior */
    ...
    Eigen::SparseMatrix<float,Eigen::ColMajor> markedRHS( markedLength , totalLabels -1);
    ...
    Eigen::SparseMatrix<float,Eigen::ColMajor> BTxM( markedLength , totalLabels-1 );

    // Build BT
    // Compare whether NumCols < NumRows for less iterations during BT building
    if ( markedIdx.size() < unmarked.size() )
    {
      Eigen::SparseMatrix<float,Eigen::ColMajor> BT( unmarkedLength , markedLength );
    ...
    else
    {
      ...
      Eigen::SparseMatrix<float , Eigen::RowMajor> BT( unmarkedLength , markedLength );
      BT.reserve(Eigen::VectorXi::Constant( markedLength , 6 ));
    ...
    Eigen::SparseMatrix<float , Eigen::RowMajor> Lu( unmarkedLength , unmarkedLength );
    ...
    // Make RHS dense
    Eigen::MatrixXf BTxMDense = Eigen::MatrixXf(BTxM);
    ...
    Eigen::BiCGSTAB <Eigen::SparseMatrix<float , Eigen::RowMajor> , Eigen::DiagonalPreconditioner<float> > solver(Lu); // Usually faster
    ...
    probabilities = solver.solve(BTxMDense);

    Lu.resize(0,0);
    Lu.data().squeeze();
    BTxMDense.resize(0,0);

    std::vector< int > RWLabels( unmarkedLength );
    ...
    for (int i = 0 ; i != unmarkedLength ; ++i )
    {
      ...
      RWLabels.at(i) = 1 - accumulatedProbability > maxProbability ?  nameLabels.back() : nameLabels.at( maxLabelPos );
    }
